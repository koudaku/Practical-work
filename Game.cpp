...
"0                 ckkck                                                                                          kkkk                                 0",
"0                                                                                           ckkk                                               r      0",
"0                                kkkkkkkkkkk                                                          w                                        rr     0",
"0                                                                    kkkkkk                                                                    rrr    0",
"0            kkkk       kckck                            c                     k                   kk   k   kkkkk                              rrrr   0",
"0                                                            t0                     kkkkkk                              t0           c         rrrrr  0",
"0G                                                  t0       00                G                                        00                     rrrrrr 0",
"0           d    g       d                          00       00                              d                          00   d      d     d    rrrrrrr0",
   ...
   void Collision(int num)
{
	...
	for (int i = rect.top/16 ; i<(rect.top+rect.height)/16; i++)  
		for (int j = rect.left/16; j<(rect.left+rect.width)/16; j++)
			 {
				 if ((TileMap[i][j]=='P')  || (TileMap[i][j]=='0') || (TileMap[i][j]=='r') || (TileMap[i][j]=='t')||(TileMap[i][j]=='c'))  
				                         {                                                                                                 
												   if (dy>0 && num==1)
				 ...
				 if (TileMap[i][j]=='k') {
												   ...
												   { rect.top =   i*16 -  rect.height;  dy=0;   onGround=true; }
											       if (dy<0 && num==1)
												   { rect.top = i*16 + 16;   dy=0;TileMap[i][j]=' ';}
												   if (dx>0 && num==0)
												   { rect.left =  j*16 -  rect.width; }
											       if (dx<0 && num==0)
												   { rect.left =  j*16 +16;}
...
int main()
{  
    RenderWindow window(VideoMode(860, 250), "Mario"); 
    ...
    { 
	    ...
	    if (Keyboard::isKeyPressed(Keyboard::W))	if (Mario.onGround) { Mario.dy=-0.27; Mario.onGround=false;  sound.play();} 
		 ...
		 if (Mario.rect.left>299) offsetX = Mario.rect.left-299;
